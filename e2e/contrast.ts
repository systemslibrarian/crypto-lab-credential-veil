 ...
 *
 *  - text over a surface axe declines to resolve. Here that is `color-mix()`:
 *    the hero aside `.cl-hero-why` is
 *    `color-mix(in oklab, var(--accent) 6%, transparent)` over the page
 *    background, and the shared top bar's `--cl-ink` — the ink on every
 *    `.cl-btn` and on `.cl-badge` — is
 *    `color-mix(in srgb, var(--accent) 60%, #eafff8)`. Chromium hands both to
 *    `getComputedStyle` unconverted, axe files them under `incomplete`, and a
 *    violations-only gate therefore measured neither.
 ...
 *    at `.85` and `#app button:disabled` at `.55`. The disabled case is not
 *    academic: every exhibit control on this page ships disabled and enables
 *    only once the issuer keys exist, so the whole pre-unlock state is painted
 *    through an opacity group.
 *
 ...
 *    boxes here are `.hexblock` (`max-height: 9.5rem; overflow: auto`, which
 *    every proof, signature and commitment on the page lands in and overflows
 *    immediately — a BBS proof is hundreds of hex characters) and `.table-wrap`
 *    (`overflow-x: auto` around a `min-width: 44rem` table, so at 380px most of
 *    both comparison tables is clipped away). Content scrolled past the client
 *    box is not dimmed or partly drawn — it is absent from the frame, and asking
 ...
 *    through to white, inventing a failure. Skip it; the part that IS in view is
 *    measured for real, in both viewports.
 *
 ...
 *    draws no labelled SVG figures — its only SVG is the shared header's two
 *    `aria-hidden` icons, which carry no `<text>` — so the sibling walk finds
 ...
 * not one — nor is it one to axe's `color-contrast` rule. This page's only
 * generated content is the UA disclosure triangle on the three `<summary>`
 * elements (the stylesheet does not set `list-style: none`, so the marker is
 * the UA's own and is painted in `currentColor`), plus the `.scope-note`
 * left rule, which is a `border-left`, not a pseudo-element. Both were measured
 * by hand from screenshot pixels; neither is regression-tested here.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * accent into surfaces and inks with `color-mix()` in BOTH `oklab` (the
     * hero's "why it matters" wash) and `srgb` (the shared bar's `--cl-ink`,
     * its border, and every `.cl-btn` hover fill), and Chromium reports those to
     * `getComputedStyle` unchanged rather than converting them to sRGB. A regex
     * that only understands `rgb()/rgba()` sees `null` for every one of them and
     * the walk then falls through to the wrong backdrop — which fabricates
     * failures (a dark-on-light header button read as dark-on-dark) and, far
     ...
     * wherever the text sits, which is only right for a gradient that spans its
     * element; a decorative wash that fades to `transparent` well before the
     * text would otherwise invent a backdrop nothing sits on. So each gradient
     ...
     *
     * This page ships no gradients today. The sampling is kept because it is
     * silent when unused and because the shared header is copied between labs,
     * several of which do paint one.
     */
     ...
     *
     * The revocation exhibit renders 64 status-list cells, each its own text
     * node, and every one of them walks the same handful of ancestors. Without
     ...
     * surface it happens to sit on and INVENTS failures. This page has two of
     * them: `<caption class="visually-hidden">` on each comparison table, which
     * is `position: absolute; clip: rect(0 0 0 0)` and paints nothing while
     * still reporting a 1x1 box inside a bordered `.table-wrap`. Deliberately
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has two closed `<details>` at first paint —
      // "The three revocation designs" and "What this lab is not" — and the
      // gate opens and scans both, by clicking their summaries.
      if ((el as HTMLElement).checkVisibility?.() === false) return false;
      const r = rectOf(el);
      if (r.width <= 0 || r.height <= 0) return false;
      // Text parked off the top edge of the page paints no pixels. This is the
      // WCAG-sanctioned "visually hidden until focused" idiom, used here by the
      // shared header's `.cl-skip-link`, which parks at `top: -3rem` and slides
      // to `top: 0` only on focus. Measuring the parked copy invents a failure
     ...
     * page has no `aria-hidden` text today — its two `aria-hidden` nodes are the
     * shared header's icon `<svg>`s, which carry shapes and no characters — so
     * the guard is silent here and is kept for the same reason axe keeps it.
     */
     ...
     * lab builds no SVG from templates, so the guard is silent here.
     */
